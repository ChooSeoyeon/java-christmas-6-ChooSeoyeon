...
- 계획 (Plan)
    ...
    - 계획(내지 계획표)는 아래와 같이 구성된다. (TODO: 추후 이미지 추가할 예정, 일단은 아래 부록의 `B-1.초안` 참고)

## 🗂️ Model

### Plan (계획)

- [ ]  방문 날짜를 표시한 `달력`을 담아 계획을 초기화한다
- [ ]  계획에 주문 메뉴를 표시한 `메뉴판을 추가`한다
- [ ]  계획에 이벤트 내역을 표시한 `이벤트를 추가`한다
- [ ]  `계획 결과 내역(dto)`을 생성한다
    - [ ]  `방문 날짜 내역(dto)`, `주문 내역(dto)`, `이벤트 내역(dto)`을 가진다

### Calendar (달력)

- [ ]  `12월 달력`과 `오늘 날짜`를 담아 달력을 초기화한다
- [ ]  12월 달력에 `방문할 날짜를 표시`한다
    - [ ]  `12월의 날짜가 아닌 경우`, 예외를 발생시킨다
    - [ ]  `이미 지난 날짜인 경우`, 예외를 발생시킨다
- [ ]  `방문 날짜 내역(dto)`를 생성한다

### Menu (메뉴판)

- [ ]  `메뉴 목록`을 담아 메뉴판을 초기화한다
- [ ]  메뉴 목록에 `주문할 메뉴를 표시`한다
    - [ ]  `없는 메뉴인 경우`, 예외를 발생시킨다
    - [ ]  `메뉴에 중복이 있는 경우`, 예외를 발생시킨다
    - [ ]  `메뉴에 음료만 있는 경우`, 예외를 발생시킨다
    - [ ]  `각 메뉴 개수가 1개 이상이 아닌 경우`, 예외를 발생시킨다
    - [ ]  `메뉴 개수 합이 20개를 초과한 경우`, 예외를 발생시킨다
- [ ]  `주문 내역(dto)`을 생성한다
    - [ ]  `주문 메뉴`, `할인 전 총주문 금액`을 가진다

### Event (이벤트)

- [ ]  `이벤트 목록`을 담아 이벤트를 초기화한다
- [ ]  이벤트 목록에 `적용되는 이벤트를 표시`한다
- [ ]  `혜택 금액을 계산`한다
- [ ]  `이벤트 내역(dto)`을 생성한다
    - [ ]  `증정 메뉴`, `혜택 내역`, `총혜택 금액`, `할인 후 예상 결제 금액`, `배지 이름`을 가진다

## 👩🏻‍💻 View

### InputView

- [ ]  `예상 방문 날짜를 입력` 받는다 (`String` → `int`)
    - [ ]  `날짜 형식에 맞지 않는 경우`, 예외를 발생시킨다
- [ ]  `주문 메뉴와 개수를` 쉼표로 구분해 `입력` 받는다 (`String` → `Map<String, Integer>`)
    - [ ]  `주문 형식에 맞지 않는 경우`, 예외를 발생시킨다

### OutputView

- [ ]  `이벤트 플래너 시작 문구`를 출력한다
- [ ]  `이벤트 혜택 미리보기 문구`를 출력한다
- [ ]  `계획 결과 내역`을 출력한다
    - [ ]  `주문 메뉴`를 출력한다
    - [ ]  `할인 전 총주문 금액`을 출력한다
    - [ ]  `증정 메뉴`를 출력한다
...
- [ ]  `이벤트 플래너를 시작`한다
    - [ ]  사용자가 입력한 `날짜에 대한 계획을 생성`한다
    - [ ]  사용자가 입력한 `메뉴를 계획에 추가`한다
    - [ ]  `이벤트를 계획에 적용`한다
    - [ ]  `계획 결과 내역을 출력`한다
...
- [ ]  입력값인 `예상 방문 날짜`와 관련된 예외를 한데 모아 처리한다
    - view
        - 사용자가 입력한 값이 `닐짜 형식에 맞지 않는 경우`
    - model
        - 사용자가 입력한 날짜가 `12월의 날짜가 아닌 경우`
        - 사용자가 입력한 날짜가 `이미 지난 날짜인 경우`

### MenuOrderException

- [ ]  입력값인 `주문 메뉴 및 개수`와 관련된 예외를 한데 모아 처리한다
    - view
        - 사용자가 입력한 값이 `주문 형식에 맞지 않는 경우`
    - model
        - 사용자가 입력한 메뉴가 `없는 메뉴인 경우`
        - 사용자가 입력한 `메뉴에 중복이 있는 경우`
        - 사용자가 입력한 `메뉴에 음료만 있는 경우`
        - 사용자가 입력한 `각 메뉴 개수가 1개 이상이 아닌 경우`
        - 사용자가 입력한 `메뉴 개수 합이 20개를 초과한 경우`
...
- 역할 : 책임의 집합 (재사용성) (❗특이점❗ : 요구사항에 맞춰 `예약 시스템` → `이벤트 플래너 시스템`으로 대체)
    ...
    - 예약 → 계획 (`Plan`)
        - 결과물 : 예약 내역 → 계획 결과 내역 (`PlanResult`)
    - 달력 (`Calendar`)
        - 결과물 : 방문 날짜 내역 (`VisitDate`)
    - 메뉴판 (`Menu`)
        - 결과물 : 주문 메뉴 내역 (`OrderedMenu`)
    - 이벤트 (`Event`)
